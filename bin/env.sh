    ...
    find "${ENV_GENERATED}" -mindepth 2 -maxdepth 2 -type d | while read -r _gen_env_dir_generated; do
        ...
        case "${_gen_env_dir}" in
          rhel|rhel_fedora)
            ...
            ;;
          *)
            if [ "${_gen_env_dir_generated##*/}" = alias.d ]; then
...
EOF
    ...
    _echo_env_sh_export ENV_SUPPORTED "arch 00-common Darwin debian fedora Linux rhel rhel_fedora \
${HOSTNAME=$(hostname)}"
...
#
# Env etc/*.d have been sourced already
: "\${ENV_ETC_D_SOURCED=0}"; export ENV_ETC_D_SOURCED

if test "\${ENV_ETC_D_SOURCED}" -eq 0; then
  export ENV_ETC_D_SOURCED=1
EOF
...
gen_env_sh_etc_rc_d_source_old() {
  $_ENV_CHANGED || return 0
